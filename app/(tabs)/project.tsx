...
import { canDelete, canManage, canViewBudget, isOwner, mergeOwnedRecords } from '@/services/permissions';
              ...
              <View key={p.id} style={s.card}>
                ...
                {(canManage(user, p) || canDelete(user, p)) && (
                  <View style={s.actions}>
                    ...
                    {canDelete(user, p) && <TouchableOpacity onPress={() => handleDelete(p.id)} style={s.delBtn}><Ionicons name="trash-outline" size={14} color="#dc2626" /></TouchableOpacity>}
                  </View>
                )}
              </View>
            );
